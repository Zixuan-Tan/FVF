...
def test(args, model, tokenizer, test_dataset: TextDataset, best_threshold=0.5):
    # build dataloader
    ...
    y_trues = np.concatenate(y_trues, 0)
    y_preds = logits[:, 1] > best_threshold
    corrspd_df_indices = [exp.index for exp in test_dataset.examples]
    np.save(os.path.join(args.output_dir, "test_y_trues.npy"), y_trues)
    np.save(os.path.join(args.output_dir, "test_y_preds.npy"), y_preds)
...
def main():
    ...
    if args.use_non_pretrained_model:
        enc = RobertaForSequenceClassification(config=config)
    else:
        enc = RobertaForSequenceClassification.from_pretrained(
            args.model_name_or_path, config=config, ignore_mismatched_sizes=True
        )
    model = Model(enc, config, tokenizer, args)
    ...
    if args.eval_export:
        model.load_state_dict(torch.load(args.model_name, map_location=args.device), strict=False)
        model.to(args.device)
        ...
        result_df.to_csv(
            os.path.join(
                os.path.dirname(args.train_data_file),
                f"eval_export_{args.seed}_{args.model_name.replace('/', '-')}.csv",
            )
        )
