...
def generate_w2v(
    ...
    model = train_w2v(lines, embedding_size=100, epochs=MAX_ITER, window_size=10)
    model.save(str(savedir / "w2v.model"))


def train_w2v(sentences, embedding_size=100, epochs=5, window_size=10):
    print("Total Examples:", len(sentences))
    wvmodel = Word2Vec(
        sentences, workers=16, vector_size=embedding_size, window=window_size
    )
...
class MyWord2Vec:
    ...
    def get_embeddings(self, sentence: str, emb_size: int = 100) -> list:
        ...
        if len(tokens):
            return np.mean(
                [
                    self.model.wv[i] if i in self.model.wv else np.zeros(100)
                    for i in tokens
                ],
                axis=0,
            ).astype(np.float32)
