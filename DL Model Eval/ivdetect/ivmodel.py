...
class IVDetect(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_conv_layers=3, dropout=0.5):
        ...
        #         exec('self.conv_{} = GraphConv(hidden_size, hidden_size)'.format(i))
        #     if i == self.layer_num - 1:
        #         exec('self.conv_{} = GraphConv(hidden_size, 2)'.format(i))
    ...
    def forward(self, g: dgl.DGLGraph, dataset, e_weights=[]):
        """Forward pass.
        ...
        # print('nodes:', nodes)
        data = dict()
        asts: list[dgl.DGLGraph] = []
        for sampleid in set(g.ndata["_SAMPLE"].detach().cpu().int().numpy()):
        ...
        # for i in range(self.layer_num):
        #     if i < self.layer_num - 1:
        #         feat_vec = eval('self.conv_{}(g, feat_vec)'.format(i))
        #         feat_vec = self.relu(feat_vec)
        #     if i == self.layer_num - 1:
        #         conv_output = eval('self.conv_{}(g, feat_vec)'.format(i))
        # g.ndata["h"] = conv_output
        ...
        #
        #     pooled = self.pool(g_i, conv_output)
        #     batch_pooled = torch.cat([batch_pooled, pooled])
        # print(g, g.ndata["h"])
        # batch_pooled = AvgPooling(g, g.ndata["h"])
        # print('before batch_pooled.shape', batch_pooled.shape)
        # batch_pooled = self.max_pool(g, g.ndata["h"])
        batch_pooled = self.avg_pool(g, g.ndata["h"])

        # batch_pooled = nn.Softmax(dim=1)(batch_pooled)
        # print('after batch_pooled.shape', batch_pooled.shape)
        return batch_pooled
